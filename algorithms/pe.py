...
class SHAC:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
            ...
            mu = torch.tanh(actions)


            # obs, rew, done, extra_info = self.env.step(mu)
            

            # def compute_fake_loss(mu):
            a = torch.tensor(mu.detach().cpu().numpy(), device=self.device, requires_grad=True)
            obs, rew, done, extra_info = self.env.step(a)
            ...
            Q = rew + self.gamma * self.target_critic(obs).squeeze(-1)
            Q = Q.sum()
            ...
            with torch.no_grad():
                pQ_pa = a._grad.detach()

            fake_loss = (pQ_pa * mu).sum(-1)
            # fake_loss = self.target_critic(obs).squeeze(-1)
            
            # next_values[i + 1] = fake_loss
            self.actor_optimizer.zero_grad()
            ...
            if i < self.steps_num - 1:
                ...
                actor_loss = actor_loss + (- rew_acc[i + 1, done_env_ids] - self.gamma * gamma[done_env_ids] * fake_loss[done_env_ids]).sum()
            else:
                # terminate all envs at the end of optimization iteration
                # actor_loss = actor_loss + (- rew_acc[i + 1, :] - self.gamma * gamma * next_values[i + 1, :]).sum()
                actor_loss = actor_loss + (- rew_acc[i + 1, :] - self.gamma * gamma * fake_loss).sum()
